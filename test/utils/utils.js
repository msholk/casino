...
async function diamondInit1() {
  ...
  const playersFacet = await ethers.getContractAt(
    "PlayersFacet",
    diamondAddress
  );
...
async function checkFacets2(facets) {
  ...
  result = await diamondLoupeFacet.facetFunctionSelectors(
    facetAddress.StakerFacet
  );
  assert.sameMembers(result, selectors);
  assert.sameKeys(
    facetSelectors.StakerFacet,
    selectorsToDic(selectors),
    "StakerFacet expected functions",
    "StakerFacet deployed functions"
  );
  facetSelectors.StakerFacet = {
    ...facetSelectors.StakerFacet,
    ...selectorsToDic(selectors),
  ...
  assert.sameMembers(result, selectors);
  assert.sameKeys(
    facetSelectors.AdminFacet,
    selectorsToDic(selectors),
    "AdminFacet expected functions",
    "AdminFacet deployed functions"
  );
  facetSelectors.AdminFacet = {
    ...facetSelectors.AdminFacet,
    ...selectorsToDic(selectors),

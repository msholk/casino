...
interface IVRF {
  ...
  function getReqID() external view returns (uint256);

  function getRnd() external view returns (uint256);
...
contract VRfTester {
  ...
  constructor(address vrfManagerAddress) {
    vrf = IVRF(vrfManagerAddress);
  }
  ...
  function check() public view returns (uint256 req, uint256 rnd) {
    return (vrf.getReqID(), vrf.getRnd());
  }

  function isAuthorized() public view returns (bool) {
    return vrf.isAuthorized();
  }
}

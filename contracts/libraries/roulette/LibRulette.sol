...
library LibRulette {
    ...
    returns (uint256 amount2Lock)
  {
    ...
    for (uint256 index; index < betPointsLength; ++index) {
      ...
      if (betType == 1) {
        LockAmountPerBetType[1] = max(LockAmountPerBetType[1], p.amount);
      }
      ...
      //3 horizontal: are mutually exclusive
        ...
        LockAmountPerBetType[3] = max(LockAmountPerBetType[3], p.amount);
      }
      //4Square: just sum
      else if (betType == 4) {
      ...
      //7:dozens: are mutually exclusive
        ...
        LockAmountPerBetType[7] = max(LockAmountPerBetType[7], p.amount);
      }
      //8:vertical line: are mutually exclusive
      else if (betType == 8) {
        LockAmountPerBetType[8] = max(LockAmountPerBetType[8], p.amount);
      }
      //9:first half: unique
      else if (betType == 9) {
    ...
    for (uint256 index = 1; index <= 13; ++index) {
      amount2Lock += LockAmountPerBetType[index] * LockFactorPerBetType[index];
    }
  }
    ...
    returns (uint256)
  {
    ...
    for (uint256 index; index < checkNums.length; ++index) {
      if (checkNums[index] == winNum) {
        return winFactor;
      }
    }

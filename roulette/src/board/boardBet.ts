...
export default class BoardBet extends Phaser.GameObjects.Image {
    ...
    constructor(scene: MainScene, bet: betInfo) {
        ...
        let chipValue = scene.bottomPannelUi.chipContainer.currentValue
        super(scene, x, y, `${chipValue}_chip`)
        ...
        // flash message with bet quantity
        let ed = EventDispatcher.getInstance()
        ed.once('betPlaced', () => {
            scene.messageFlasher.flashMessage(this.x + scene.board.x, this.y + scene.board.y, `+${chipValue}`, '2rem', scene.messageFlasher.colors.INFO)
        })
    }
